...
class Index extends Component {
  ...
  butttonTwo = () => {
    let { name, date, amount, errorName, errorAmount, errorDate, canGoToStep1, invoiceAmount, repaymentDate } = this.state;
    if (canGoToStep1) {
   ...
   if ( name != '' && date != '' && amount != '' && errorName == '' && errorAmount == '' && errorDate == ''){
      this.setState({
        stepOne: false,
        stepTwo: true,
        canGoToStep1: false
  ...
  submitButton = () => {
    let { invoiceName, repaymentDate, issueDate, invoiceAmount, errorInvoiceAmount, errorRepaymentDate, errorIssueDate, errorInvoiceName  } = this.state;
    if(invoiceName == '') {
      this.setState({
    ...
    if ( invoiceName != '' && repaymentDate != '' && issueDate != '' && invoiceAmount != '' 
      && errorInvoiceName == '' && errorRepaymentDate == '' && errorInvoiceAmount == '' && errorIssueDate == '') {
      this.setState({
        submitted: true,
      });
    }
  ...
  onInvoiceAmount = (invoiceAmount) => {
    ...
    if(invoiceAmount > amount) {
      this.setState({invoiceAmount : invoiceAmount, canGoToStep1: false, errorInvoiceAmount: ''});   
    } else {
      this.setState({invoiceAmount : invoiceAmount, canGoToStep1: true, errorInvoiceAmount: 'Amount should be more than the Deal else change the Deal amount'});   
    }
  }
  onInvoiceName = (invoiceName) => {
    if(/^[A-Za-z]+$/.test(invoiceName)){
      ...
      this.setState({invoiceName : invoiceName, errorInvoiceName: 'Please enter only alphabets'});
    }  
  }
  onRepaymentDate = (repaymentDate) => {
    ...
    this.setState({repaymentDate : repaymentDate, canGoToStep1: true, errorRepaymentDate: 'Listing date should be in b/w issuing & repayment date, please change listing date in step1'});  
    }
  }
